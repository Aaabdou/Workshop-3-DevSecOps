...
public class Main {
    public static void main(String[] args) {
        ...
        while (username == null) {
            ...
            if (choix == 1 && authService.login(user, pass)) {
                username = user;
                taskService = new TaskServices(username);
                System.out.println("Successful connection !");
            } else if ((choix == 2) && authService.register(user, pass)) {
                username = user;
                taskService = new TaskServices(username);
